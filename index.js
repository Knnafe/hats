...
bot.on("ready", async () => {
  ...
  bot.user.setActivity(`Hats Clients | >help  `, {type: "WATCHING"});

});

bot.on('guildMemberAdd', member => {
  let channel = member.guild.channels.find('name', 'hats-heaven-door');
...
bot.on('guildMemberRemove', member => {
  let channel = member.guild.channels.find('name', 'hats-graveyard');
  let memberavatar = member.user.avatarURL
      if (!channel) return;
      let embed = new Discord.RichEmbed()
      .setColor('RANDOM')
